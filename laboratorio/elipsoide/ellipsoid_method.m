...
if p.Results.numiter == 0
    %N = 2*n*((2*n+1)*input_size(C)+n*input_size(d)-n^3); %according to Gr�tschel 3.1.32
    N = 50 * (n+1)^2 * input_size(C,d); %according to Gr�tschel 3.1.37
...
if p.Results.radius == 0
    R = max( n * 2^(2*(input_size(C,d)-n^2)) , 1); %according to Gr�tschel 3.1.32 & p. 80 avoid sqrt!
...
A = R * eye(n); % take R^2 above to avoid calc of sqrt in line 32 (see Gr�tschel p. 80)
...
for i=1:N
    ...
    % deep cut according to Gr�tschel 3.1.42
...
%     recalculate variables (L�wner-John-Ellipsoid, central cut)
%     b = (A * c) / (sqrt(c' * A * c));
%     a = a - b/(n+1);
%     A = (n^2 / (n^2-1)) * (A -  (2/(n+1)) * (b * b'));
    ...
    % output all variables for thesis
     printf('\n %d  ',i)
     printf('\t D = [ %3.f , %3.5f ; %3.5f , %3.5f ]', A)
     printf('\t a = [ %3.5f , %3.5f ]', a )
     printf('\t x[i] = [ %.5f , %.5f ]', best_a )
     
     printf('\t -> Valor objetivo = %5.10f', best_a(1) * o(1) + best_a(2) * o(2)  )
     %printf('\t r = %.5f * %.5f + %.5f * %.5f = %.5f', best_a(1) ,o(1) , best_a(2) ,  o(2) , best_a(1) * o(1) + best_a(2) * o(2)  )
...
printf('\n============================================================================================================================================================' )
printf('\n Cantidad de Actualizaciones = %d '  ,  contador );
printf('\n============================================================================================================================================================' )
